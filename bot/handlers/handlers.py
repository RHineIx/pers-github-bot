...
from ..utils import TrackCommandParser
...
class BotHandlers:
    ...
    def register_handlers(self):
        ...
        self.bot.message_handler(is_owner=True, commands=["start", "help"])(
            self.handle_help
        )
        self.bot.message_handler(is_owner=True, commands=["status"])(self.handle_status)
        self.bot.message_handler(is_owner=True, commands=["testlog"])(
            self.handle_test_log
        )

        # Register Token Management commands
        self.bot.message_handler(is_owner=True, commands=["settoken"])(
            self.handle_set_token
        )
        self.bot.message_handler(is_owner=True, commands=["removetoken"])(
            self.handle_remove_token
        )

        # Register Destination Management commands
        self.bot.message_handler(is_owner=True, commands=["add_dest"])(
            self.handle_add_destination
        )
        self.bot.message_handler(is_owner=True, commands=["remove_dest"])(
            self.handle_remove_destination
        )
        self.bot.message_handler(is_owner=True, commands=["list_dests"])(
            self.handle_list_destinations
        )
...
"""
    ...
    async def handle_test_log(self, message: Message):
        ...
        try:
            ...
            await self.bot.reply_to(
                message, "✅ A test error log has been sent to the log channel."
            )
        except Exception as e:
        ...
        try:
            ...
            if self.scheduler and self.scheduler.scheduler.running:
                ...
                if jobs:
                    ...
                    if next_run_time:
                        next_run_info = next_run_time.strftime(
                            "%A, %Y-%m-%d at %H:%M:%S %Z"
                        )
            ...
            if res.get("user"):
                status_text += (
                    f"👤 *GitHub Account:* `@{res['user'].get('login', 'N/A')}`\n"
                )
            ...
            current_interval = res.get("interval") or config.STARS_MONITOR_INTERVAL
            status_text += (
                f"⏱️ *Check Interval:* `{format_duration(current_interval)}`\n"
            )
            status_text += (
    ...
    async def handle_set_token(self, message: Message):
        ...
        try:
            ...
            await self.bot.reply_to(
                message, "Usage: `/settoken <your_token>`", parse_mode="Markdown"
            )
            return
        ...
        try:
            ...
            if not user_data or "login" not in user_data:
                raise GitHubAPIError(401, "Invalid token")
            ...
            await self.bot.reply_to(
                message, "❌ **Invalid Token.**", parse_mode="Markdown"
            )
        finally:
            try:
    ...
    async def handle_add_destination(self, message: Message):
        ...
        try:
            ...
            if not (
                target_id.startswith("-") or target_id.replace("/", "").isnumeric()
            ):
        ...
        wait_msg = await self.bot.reply_to(
            message, f"Verifying destination `{target_id}`..."
        )
        try:
            ...
            await self.bot.edit_message_text(
                f"✅ Destination `{target_id}` added successfully!",
                wait_msg.chat.id,
                wait_msg.message_id,
                parse_mode="Markdown",
            )
            ...
            await self.bot.edit_message_text(
                error_text, wait_msg.chat.id, wait_msg.message_id, parse_mode="Markdown"
            )
    ...
    async def handle_remove_destination(self, message: Message):
        ...
        try:
            ...
            await self.bot.reply_to(
                message, "Usage: `/remove_dest <ID|me>`", parse_mode="Markdown"
            )
            return
        ...
        if rows_affected > 0:
            await self.bot.reply_to(
                message, f"✅ Destination `{target_id}` removed.", parse_mode="Markdown"
            )
        else:
            await self.bot.reply_to(
                message,
                f"❌ Destination `{target_id}` not found.",
                parse_mode="Markdown",
            )
    ...
    async def handle_list_destinations(self, message: Message):
        ...
        if not destinations:
            await self.bot.reply_to(
                message, "There are no notification destinations configured."
            )
            return
            ...
            if query_text.startswith(".repo "):
                ...
                if parsed:
                    ...
                    tasks = {
                        ...
                        "languages": self.github_api.get_repository_languages(
                            owner, repo_name
                        ),
                    ...
                    if res["repo_data"]:
                        ...
                        results.append(
                            InlineQueryResultArticle(
                                ...
                                description=res["repo_data"].get("description", "")[
                                    :60
                                ],
                                input_message_content=InputTextMessageContent(
                                    preview_text, parse_mode="HTML"
                                ),
                                thumbnail_url=res["repo_data"]
                                .get("owner", {})
                                .get("avatar_url"),
                            )
                        )
    ...
    # Add these new methods to the BotHandlers class

    async def handle_track_release(self, message: telebot.types.Message):
        ...
        if not parsed_args:
            await self.bot.reply_to(
                message,
                "❌ **Invalid Format.**\nUse: `/track_rel owner/repo`\nOr: `/track_rel owner/repo > chat_id/thread_id`",
            )
            return

        repo_full_name = parsed_args["repo_full_name"]
        chat_id = parsed_args["chat_id"] or str(message.chat.id)
        ...
        try:
            ...
            await self.bot.edit_message_text(
                f"❌ Error verifying repository: {e}",
                wait_msg.chat.id,
                wait_msg.message_id,
            )
            return

        # Verify bot permissions if destination is not the user's DM
        if str(chat_id) != str(message.chat.id):
            try:
                await self.bot.edit_message_text(
                    "🔐 Verifying permissions in destination chat...",
                    wait_msg.chat.id,
                    wait_msg.message_id,
                )
                member = await self.bot.get_chat_member(chat_id, self.bot.user.id)
                if member.status not in ["administrator", "member"]:
                    raise Exception("Bot is not a member of the chat.")
            except Exception:
                await self.bot.edit_message_text(
                    f"❌ **Permission Denied.**\nI can't send messages to `{chat_id}`. Please make sure I am a member of the group/channel and have permission to post.",
                    wait_msg.chat.id,
                    wait_msg.message_id,
                )
                return
        ...
        await self.bot.edit_message_text(
            f"💾 Saving subscription for `{repo_full_name}`...",
            wait_msg.chat.id,
            wait_msg.message_id,
        )
        success = await self.db_manager.add_release_subscription(
            repo_full_name, chat_id, thread_id
        )

        if success:
            dest_text = (
                f"topic `{thread_id}` in chat `{chat_id}`"
                if thread_id
                else f"chat `{chat_id}`"
            )
            await self.bot.edit_message_text(
                f"✅ **Now tracking releases for `{repo_full_name}`.**\nNotifications will be sent to {dest_text}.",
                wait_msg.chat.id,
                wait_msg.message_id,
            )
        else:
            await self.bot.edit_message_text(
                f"⚠️ This subscription already exists for `{repo_full_name}`.",
                wait_msg.chat.id,
                wait_msg.message_id,
            )

    async def handle_untrack_release(self, message: telebot.types.Message):
        """Handles the /untrack_rel command."""
        parsed_args = TrackCommandParser.parse_track_command(
            message.text.replace("/untrack_release", "/track_release")
        )  # Reuse parser

        if not parsed_args:
            await self.bot.reply_to(
                message,
                "❌ **Invalid Format.**\nUse: `/untrack_release owner/repo`\nOr: `/untrack_release owner/repo > chat_id/thread_id`",
            )
            return
        ...
        success = await self.db_manager.remove_release_subscription(
            repo_full_name, chat_id, thread_id
        )
        if success:
            await self.bot.reply_to(
                message, f"✅ **Subscription removed** for `{repo_full_name}`."
            )
        else:
            await self.bot.reply_to(
                message, f"❌ **Subscription not found** for `{repo_full_name}`."
            )

    async def handle_list_tracked_releases(self, message: telebot.types.Message):
        """Handles the /list_tracked_rels command."""
        subscriptions = await self.db_manager.list_tracked_releases()
        if not subscriptions:
            await self.bot.reply_to(
                message, "ℹ️ You are not tracking any repository releases yet."
            )
            return

        text = "📋 **Tracked Repository Releases:**\n\n"
        for sub in subscriptions:
            dest_text = f" -> `{sub['destination_chat_id']}`"
            if sub["destination_thread_id"]:
                dest_text += f"/{sub['destination_thread_id']}"
            text += f"- `{sub['repo_full_name']}`{dest_text}\n"
