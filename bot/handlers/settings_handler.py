...
class SettingsHandler:
    def __init__(self, bot: AsyncTeleBot, db_manager: DatabaseManager, scheduler: "DigestScheduler"):
    ...
    def register_handlers(self):
        ...
        self.bot.callback_query_handler(is_owner=True, func=lambda call: True)(self.handle_callback_query)
    ...
    async def handle_callback_query(self, call: CallbackQuery):
        ...
        if action == 'main_menu':
            ...
            await self.db_manager.set_ai_features_enabled(not await self.db_manager.are_ai_features_enabled())
            await self._send_settings_menu(call.message.chat.id, call.message.message_id, is_edit=True)
        elif action == 'toggle_ai_media_selection':
            await self.db_manager.set_ai_media_selection_enabled(not await self.db_manager.is_ai_media_selection_enabled())
            await self._send_settings_menu(call.message.chat.id, call.message.message_id, is_edit=True)
    ...
    async def _send_settings_menu(self, chat_id, message_id=None, is_edit=False):
        ...
        keyboard.add(
            InlineKeyboardButton(ai_text, callback_data=CallbackDataManager.create_callback_data('toggle_ai_features')),
            InlineKeyboardButton(ai_media_text, callback_data=CallbackDataManager.create_callback_data('toggle_ai_media_selection'))
        )
        keyboard.add(
