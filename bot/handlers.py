...
class BotHandlers:
    ...
    def register_handlers(self):
        ...
        self.bot.message_handler(is_owner=True, commands=["setinterval"])(
        ...
        # --- Register the digest command handler ---
        self.bot.message_handler(is_owner=True, commands=["digest"])(self.handle_digest)

        # Using your custom command names for destinations
...
`/add_dest <ID>` - Adds a channel/group ID.
...
"""
        try:
            ...
            logger.warning(
                f"Could not send message with effect, sending normally. Error: {e}"
            )
    ...
    async def handle_pause(self, message: Message):
        ...
        await self.bot.reply_to(
            message, "⏸️ Monitoring has been paused.", parse_mode="Markdown"
        )

    async def handle_resume(self, message: Message):
        await self.db_manager.set_monitoring_paused(False)
        await self.bot.reply_to(
            message, "▶️ Monitoring has been resumed.", parse_mode="Markdown"
        )
    ...
    async def handle_set_interval(self, message: Message):
        ...
        try:
            ...
            if len(parts) < 2:
                await self.bot.reply_to(message, "Example: `/setinterval 300`")
                return
            seconds = int(parts[1])
            min_interval = 60
            if seconds < min_interval:
                await self.bot.reply_to(
                    message, f"❌ Minimum interval is {min_interval} seconds."
                )
                return
            ...
            await self.bot.reply_to(
                message,
                f"✅ Monitoring interval set to *{human_readable_duration}*.",
                parse_mode="Markdown",
            )
        except ValueError:
            await self.bot.reply_to(message, "❌ Please enter a valid number.")
        except Exception as e:
            logger.error(f"Error in handle_set_interval: {e}")
            await self.bot.reply_to(message, "An error occurred.")
    ...
    async def handle_set_token(self, message: Message):
        ...
        if token.startswith("/settoken"):
            await self.bot.reply_to(message, "Example: `/settoken ghp_...`")
            return
    ...
    async def handle_digest(self, message: Message):
        """Handles setting the digest mode."""
        try:
            # Extract the mode from the command, e.g., 'daily', 'weekly', 'off'
            ...
            reply_message = f"✅ Digest mode has been set to *{mode}*."
            if mode == "off":
                reply_message = "✅ Instant notifications are now **ON**."

            await self.bot.reply_to(message, reply_message, parse_mode="Markdown")
        except (IndexError, ValueError):
            # This runs if the user sends /digest without an argument or with a wrong one
            await self.bot.reply_to(
                message, "Usage: `/digest <daily|weekly|off>`", parse_mode="Markdown"
            )

    async def handle_status(self, message: Message):
        """Handles the /status command, now with error and digest mode display."""
        if not await self.db_manager.token_exists():
            await self.bot.reply_to(message, "No GitHub token is set.")
            return
        ...
        try:
            ...
            status_text += (
                f"🔔 *Notification Mode:* `{res['digest_mode'].capitalize()}`\n"
            )
            ...
            current_interval = (
                res["interval"] if res["interval"] else config.MONITOR_INTERVAL_SECONDS
            )
            status_text += (
                f"⏱️ *Check Interval:* `{format_duration(current_interval)}`\n"
            )
            status_text += (
                f"📍 *Destinations:* `{len(res['destinations'])}` configured."
            )
            ...
            await self.bot.edit_message_text(
                "❌ An error occurred.", wait_msg.chat.id, wait_msg.message_id
            )

    async def handle_add_destination(self, message: Message):
        """Adds a destination with proactive permission check."""
        if not await self.db_manager.token_exists():
            await self.bot.reply_to(message, "Please set a GitHub token first.")
            return
        parts = message.text.split(" ", 1)
        target_id_str = (
            str(message.from_user.id) if len(parts) == 1 else parts[1].strip()
        )
        ...
        if target_id_str.startswith("-"):  # Check only for groups/channels
            try:
                test_msg = await self.bot.send_message(
                    target_id_str,
                    "✅ Bot permission test successful. This message will be deleted.",
                )
                await self.bot.delete_message(test_msg.chat.id, test_msg.message_id)
            except ApiTelegramException as e:
                logger.error(
                    f"Permission test failed for {target_id_str}: {e.description}"
                )
                await self.bot.reply_to(
                    message,
        ...
        await self.bot.reply_to(
            message, f"✅ Destination added: `{target_id_str}`", parse_mode="Markdown"
        )
    ...
    async def handle_remove_destination(self, message: Message):
        parts = message.text.split(" ", 1)
        if len(parts) < 2:
            await self.bot.reply_to(message, "Please specify destination ID or 'me'.")
            return
        ...
        if rows_affected > 0:
            await self.bot.reply_to(
                message, f"✅ Destination removed: `{target_id}`", parse_mode="Markdown"
            )
        else:
            await self.bot.reply_to(
                message,
                f"❌ Destination not found: `{target_id}`",
                parse_mode="Markdown",
            )
    ...
    async def handle_list_destinations(self, message: Message):
        ...
        if not destinations:
            await self.bot.reply_to(
                message, "No notification destinations are configured."
            )
            return
                ...
                if parsed:
                    ...
                    if res["repo_data"]:
                        ...
                        if self.summarizer and res["readme"]:
                            ai_summary = await self.summarizer.summarize_readme(
                                res["readme"]
                            )
