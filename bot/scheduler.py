...
class DigestScheduler:
    ...
    async def _send_digest(self):
        """Fetches queued repos and sends notifications for them sequentially."""
        ...
        for repo_full_name in queued_repos:
            try:
                ...
                # We need to get the full repo data object to pass to the notification logic
    ...
    async def _process_and_send_notification(self, repo_data: dict):
        """This is the full notification logic, moved here to be reusable."""
        ...
        try:
            ...
            if selected_media_urls:
                for i, url in enumerate(selected_media_urls):
                    ...
                    if any(url.lower().endswith(ext) for ext in [".mp4", ".mov"]):
